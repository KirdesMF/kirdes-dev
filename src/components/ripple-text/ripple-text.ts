...
export class RippleTextEffect {
	...
	handleInput(e: MouseEvent) {
		...
		if (
			!this.lastSpawnPos ||
			Math.hypot(x - this.lastSpawnPos.x, y - this.lastSpawnPos.y) > 40
		) {
			this.spawnWave(x, y);
		...
		for (let y = 0; y < 256; y++) {
			for (let x = 0; x < 256; x++) {
				...
				if (distance < maxRadius) {
					...
					const edgeFade = 1 - (distance / maxRadius) ** 2;
	...
	animate(deltaTime: number) {
		...
		this.app.renderer.render({
			container: this.displacementContainer,
			target: this.displacementTexture,
		});
	}
	...
	handleResize() {
		...
		const bg = this.displacementContainer.getChildByLabel(
			"background",
		) as Graphics;
		if (bg) {
			bg.clear();
