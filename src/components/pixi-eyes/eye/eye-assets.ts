...
import { BlurFilter, Container, Graphics, GraphicsContext, Rectangle, type Renderer } from "pixi.js";
...
export { HIGHLIGHT_RADIUS, IRIS_RADIUS, PUPIL_RADIUS, SCLERA_RADIUS };
...
export type SharedContexts = {
	...
	monsterPupilContext: GraphicsContext;
	slitGlobeContext: GraphicsContext;
	highlightContext: GraphicsContext;
	simpleDashedRingContext: GraphicsContext;
	simpleInnerRingContext: GraphicsContext;
};
...
export type BucketTextures = {
	...
	monsterPupilTexture: ReturnType<Renderer["generateTexture"]>;
	slitGlobeTexture: ReturnType<Renderer["generateTexture"]>;
	roundHighlightTexture: ReturnType<Renderer["generateTexture"]>;
	simpleDashedRingTexture: ReturnType<Renderer["generateTexture"]>;
	simpleInnerRingTexture: ReturnType<Renderer["generateTexture"]>;
};
...
export function resolveTextureResolutionProfile(
	...
	const largeResolution = quantizeResolution(Math.max(safeRendererResolution * 2, relativeScale * 2));
...
function createSimpleDashedRingContext(): GraphicsContext {
	const barCount = 40;
	const innerRadius = 9.85;
	const outerRadius = 15.8;
	const barHalfWidth = 0.42;
	...
	for (let index = 0; index < barCount; index += 1) {
		...
		const tangentX = -sin;
		const tangentY = cos;
		const innerLeftX = cos * innerRadius + tangentX * barHalfWidth;
		const innerLeftY = sin * innerRadius + tangentY * barHalfWidth;
		const outerLeftX = cos * outerRadius + tangentX * barHalfWidth;
		const outerLeftY = sin * outerRadius + tangentY * barHalfWidth;
		const outerRightX = cos * outerRadius - tangentX * barHalfWidth;
		const outerRightY = sin * outerRadius - tangentY * barHalfWidth;
		const innerRightX = cos * innerRadius - tangentX * barHalfWidth;
		const innerRightY = sin * innerRadius - tangentY * barHalfWidth;
...
function createSimpleInnerRingContext(): GraphicsContext {
	return new GraphicsContext().circle(0, 0, PUPIL_RADIUS * 0.78 + 1.2).stroke({
		color: 0xefe5b7,
		width: 1.4,
		alignment: 1,
	});
...
function createMonsterPupilContext(): GraphicsContext {
	const width = PUPIL_RADIUS * 0.42;
	const height = PUPIL_RADIUS * 1.08;
	return new GraphicsContext()
		.moveTo(0, -height)
		.bezierCurveTo(width, -height * 0.72, width, height * 0.72, 0, height)
		.bezierCurveTo(-width, height * 0.72, -width, -height * 0.72, 0, -height)
		.closePath()
		.fill(0xffffff);
}
...
export function createSharedContexts(): SharedContexts {
	...
	const monsterPupilContext = createMonsterPupilContext();
	...
	const simpleDashedRingContext = createSimpleDashedRingContext();
	const simpleInnerRingContext = createSimpleInnerRingContext();
	...
	return {
		...
		slitGlobeContext,
		highlightContext,
		simpleDashedRingContext,
		simpleInnerRingContext,
	};
}
...
function generateBucketTextures(
	...
	return {
		...
		roundGlobeHighlightTexture: generateTextureFromContext(renderer, contexts.roundGlobeHighlightContext, {
			resolution,
		}),
		...
		roundPupilTexture: generateTextureFromContextWithFrame(renderer, contexts.roundPupilContext, pupilFrameSize, {
			resolution,
		}),
		monsterPupilTexture: generateTextureFromContextWithFrame(
			renderer,
			contexts.monsterPupilContext,
			pupilFrameSize + 4,
			{ resolution },
		),
		...
		simpleDashedRingTexture: generateTextureFromContext(renderer, contexts.simpleDashedRingContext, {
			resolution: resolution * 2,
		}),
		simpleInnerRingTexture: generateTextureFromContext(renderer, contexts.simpleInnerRingContext, {
			resolution: resolution * 2,
		}),
	};
}
...
export function destroySharedContexts(contexts: SharedContexts): void {
	...
	contexts.monsterPupilContext.destroy();
	contexts.slitGlobeContext.destroy();
	contexts.highlightContext.destroy();
	contexts.simpleDashedRingContext.destroy();
	contexts.simpleInnerRingContext.destroy();
}
...
export function destroySharedTextures(textures: SharedTextures): void {
	for (const bucket of Object.values(textures.buckets)) {
		...
		bucket.monsterPupilTexture.destroy(true);
		bucket.slitGlobeTexture.destroy(true);
		bucket.roundHighlightTexture.destroy(true);
		bucket.simpleDashedRingTexture.destroy(true);
		bucket.simpleInnerRingTexture.destroy(true);
	}
}
