...
export class PortfolioTissue {
	...
	async updateColor() {
		if (!this.text || !this.tilingSprite || !this.plane || !this.app.renderer)
			return;
		...
		if (oldTextTexture && oldTextTexture !== newTextTexture)
			oldTextTexture.destroy();
		if (oldRenderTexture && oldRenderTexture !== newRenderTexture)
